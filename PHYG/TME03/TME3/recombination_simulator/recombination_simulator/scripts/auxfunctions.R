...
# Draw a color scale.
# nsamples = number of rectangles to show (maximum)
#            set to NULL to display all colors
#            (this may make the PDF big if colors is big)
drawColorScale <- function(x=NULL, y=NULL, w=NULL, h=NULL, colors=defaultcolors, text=NULL, nsamples=20) {
  usr <- par("usr")
  UW <- usr[2] - usr[1]
  UH <- usr[4] - usr[3]
  if (is.null(x))
  ...
  if (is.null(h))
    h <- UH/20
  if (!is.null(nsamples) && length(colors) > nsamples) {
    mycolors <- colors[seq(1,length(colors),length.out=nsamples)]
  } else {
    mycolors <- colors
  }
...
imageAndColorScale <- function(x, col=defaultcolors, zlim=NULL, ...) {
  ...
  drawColorScale(colors=col, text=signif(seq(zlim[[1]], zlim[[2]], length.out=5), 3))
  invisible(NULL)
}
...
stringToCharArray <- function(s) {
  return(unlist(strsplit(s, "", fixed=TRUE)))
}
