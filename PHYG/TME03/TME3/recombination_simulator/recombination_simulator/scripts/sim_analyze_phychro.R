...
sink(paste(params$outDirectory, "/benchmark.txt", sep=""), split=TRUE)
...
while (TRUE) {
  ...
  phyChroResultFile <- paste(phyChroDir, "/tree.outtree", sep="")
  if (!file.exists(phyChroResultFile)) {
    break
  }
  
  phyChroTree <- read.tree(phyChroResultFile)
  ...
  d <- dist.topo(correctTreeUnrooted, phyChroTree, method="PH85")
  bi <- prop.part(correctTreeUnrooted)
  z <- prop.clades(correctTreeUnrooted, phyChroTree)
  nErrors <- sum(z == 0)
  ...
  v <- prop.clades(phyChroTree, correctTreeUnrooted)
  v <- v[a2b(2, length(v))]
  confidence <- as.numeric(phyChroTree$node.label[a2b(2, length(phyChroTree$node.label))])
...
if (simIndex == 1) {
  ...
  pdf(paste(params$outDirectory, "/plot-benchmark.pdf", sep=""), width=11, height=8)
  ...
  legend("topright", fill=c("forestgreen", "orange"), legend=paste(c(length(incorrectEdgeLengths), length(correctEdgeLengths)), c("incorrect splits","correct splits")))
  ...
  save.image(paste(params$outDirectory, "/benchmark-rdata.rda", sep=""))
